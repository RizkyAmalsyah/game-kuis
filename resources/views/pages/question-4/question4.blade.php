    ...
    <div class="container">
      <div class="header-question p-3 mt-5">
        <div class="d-flex justify-content-between">
          <div class="head-soal">4/5</div>
          <progress id="file" value="80" max="100" style="width:1000px; height:70px;"></progress>
        </div>
      </div>
      ...
      <div class="answer-content">
        <form action="{{ route('question-4-answer', $quiz->id) }}" method="post">
          @csrf
      ...
      <div class="notes">
        @if ($data->score > 0)
          <div class="alert alert-danger rounded-4 text-center">
            Ayo Lanjutkan Menjawab Soal Dengan Benar!
          </div>
        @else
          <div class="alert alert-danger rounded-4 text-center">
            Fokus Pada Soal Nomor 4 ini, Lupakan Kesalahan Jawab!
          </div>
        @endif
      </div>
      <div class="timer">
...
<script>
  ...
  setTimeout(function () {
      window.location.href= '{{ route('question-4-notime', $quiz->id) }}'; // the redirect goes here
    },10000);
</script>
@endpush
