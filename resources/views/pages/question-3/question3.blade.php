    ...
    <div class="container">
      <div class="header-question p-3 mt-5">
        <div class="d-flex justify-content-between">
          <div class="head-soal">3/5</div>
          <progress id="file" value="60" max="100" style="width:1000px; height:70px;"></progress>
        </div>
      </div>
      ...
      <div class="answer-content">
        <form action="{{ route('question-3-answer', $quiz->id) }}" method="post">
          @csrf
      ...
      <div class="notes">
        @if ($data->score > 0)
          <div class="alert alert-danger rounded-4 text-center">
            Go Go Lanjutkan Menjawab Dengan Benar, Tersisa 3 Soal Lagi!
          </div>
        @else
          <div class="alert alert-danger rounded-4 text-center">
            Hiraukan Kesalahan Sebelumnya, Fokus Pada Soal Nomor 3 ini!
          </div>
        @endif
      </div>
      <div class="timer">
...
<script>
  ...
  setTimeout(function () {
      window.location.href= '{{ route('question-3-notime', $quiz->id) }}'; // the redirect goes here
    },10000);
</script>
@endpush
